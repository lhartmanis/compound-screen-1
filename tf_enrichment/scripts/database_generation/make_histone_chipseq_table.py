...
if '__main__' == __name__:
    ...
    parser.add_argument('--folder_in', required=True)
    ...
    print(len(metadata))
    metadata = metadata[metadata['Output type'].isin(('optimal IDR thresholded peaks', 'pseudoreplicated IDR thresholded peaks', 'pseudoreplicated peaks'))]
    print(len(metadata))
    metadata = metadata[metadata['File type']=='bed']
    print(len(metadata))
    ...
    for TF, accession in zip(metadata['Experiment target'], metadata['File accession']):
        TF = o.prefix + TF.split('-human')[0] + o.suffix
